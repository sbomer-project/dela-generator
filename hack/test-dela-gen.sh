...
# Usage: ./test-dela-gen.sh [API_URL] [OPERATION_ID]

# Configuration
API_URL="${1:-http://localhost:8080}"
OPERATION_ID="${2:-MOCK-OP-123}"
ENDPOINT="${API_URL}/api/v1/generations"

echo "Target API: $ENDPOINT"
echo "PNC Operation ID: $OPERATION_ID"

# Construct JSON Payload
# Note: The target type is explicitly set to PNC_DELA to route
# the request to the Deliverables Analyzer Generator microservice.
PAYLOAD=$(cat <<EOF
{
  "generationRequests": [
    {
