...
# Variables for minikube profile and dela-generator
SBOM_SERVICE_IMAGE="dela-generator:latest"
PROFILE="sbomer"
TAR_FILE="dela-generator.tar"

echo "--- Building and inserting dela-generator image into Minikube registry ---"

bash ./hack/build-with-schemas.sh prod

podman build --format docker -t "$SBOM_SERVICE_IMAGE" -f src/main/docker/Dockerfile.jvm .

echo "--- Exporting dela-generator image to archive ---"
if [ -f "$TAR_FILE" ]; then
    rm "$TAR_FILE"
fi
podman save -o "$TAR_FILE" "$SBOM_SERVICE_IMAGE"

echo "--- Loading dela-generator into Minikube ---"
