...
LOCAL_CHART_PATH="helm/dela-generator-chart"
...
echo "--- Building local dela-generator image inside of Minikube ---"
# Will have local image dela-generator:latest in minikube
bash ./hack/build-local-dela-generator-into-minikube.sh
...
target_name = 'dela-generator-chart'
...
for line in lines:
    ...
    # If we are inside the dela-generator-chart block, replace values
    if in_block:
